...
## CBR key rate source and OFZ-PD chart

Контракт источника ключевой ставки Банка России зафиксирован в [`docs/02_data_contracts/cbr_key_rate_contract.md`](docs/02_data_contracts/cbr_key_rate_contract.md).

В целевой модели в scope входит только ключевая ставка Банка России. Инфляция, цель по инфляции и любые поля `inflation_yoy` / `inflation_target` не входят в контракт key rate.

Primary source для web parser:

```text
https://cbr.ru/hd_base/KeyRate/
```

Preferred parser source внутри HTML - `table.data` с колонками `Дата` и `Ставка`. Highcharts допускается только как fallback/cross-check. Monthly key rate выбирается как последнее доступное наблюдение месяца (`last_available_observation_in_month`).

Daily source copy хранится как нормализованная копия формы сайта Банка России, строго с колонками:

```text
date,value
```

Целевой raw latest path:

```text
data/raw/cbr/key_rate_inflation/latest/cbr_key_rate_daily.csv
```

Provenance не пишется в daily CSV. Если metadata нужна, она хранится отдельно:

```text
data/raw/cbr/key_rate_inflation/latest/cbr_key_rate_daily.meta.json
```

Registry хранится отдельно:
...
Monthly view для графика строится в памяти из raw daily source copy и содержит `key_rate_month_end_pct`, `key_rate_date`, `key_rate_source_rule=last_available_observation_in_month` и `key_rate_month_is_partial`. Production monthly CSV в `data/processed/reference/` больше не используется. Max/min/avg/first aggregations для ключевой ставки запрещены.
...
Базовый dry-run без записи raw-файлов:

```powershell
.\.venv\Scripts\python.exe scripts\reference_data\cbr_key_rate.py --source web --dry-run
```

Parser поддерживает источники `web`, `html-file` и emergency fallback `xlsx`. Для web/html источника preferred parser source - `table.data`; Highcharts используется только как fallback/cross-check. Запись выполняется только при `--download --confirm UPDATE_CBR_KEY_RATE`: parser пишет raw latest CSV/meta, version snapshot и registry в `data/raw/cbr/key_rate_inflation/`.
...
XLSX fallback остается только аварийным legacy-режимом диагностики. Он не является основным источником, не должен скрыто создавать production raw dataset в chart flow и не заменяет web source `https://cbr.ru/hd_base/KeyRate/`. Папка `data/raw/cbr/key_rate_inflation/` сохраняется как текущий raw-root ключевой ставки; inflation остается вне scope.
...
График использует только `yield_scope=ofz_pd_only`, соединяет ключевую ставку по месяцу и не интерполирует пропуски CBR ряда. Chart flow читает `data/raw/cbr/key_rate_inflation/latest/cbr_key_rate_daily.csv` и строит monthly-ряд в памяти; `data/processed/reference/cbr_key_rate_monthly.csv` не используется как production source. Если raw latest отсутствует, сначала обновите raw dataset на вкладке `Банк России`. Default source registry policy не менялась: `source-registry-mode=warn`, `allow-legacy-raw=true`.
...
Все source, tests, scripts, configs, prompts и документация проекта должны храниться в UTF-8. Invalid UTF-8 и признаки mojibake являются блокирующими дефектами для quality gate и release. Generated artifacts, caches, virtual environments, `data/processed/`, `outputs/`, `logs/` и `releases/` исключены из encoding-аудита и не коммитятся.
...
Проверка кодировки запускается отдельно или в составе `ofz-quality --fast`:
...
Команда `ofz-gui.exe` без пути работает только если `.venv\Scripts` находится в `PATH`, например после активации `.venv`.
...
Русская operator procedure: [`docs/07_operations/gui_launcher.md`](docs/07_operations/gui_launcher.md). Headless smoke: `.\.venv\Scripts\ofz-gui.exe --smoke`; static wrapper smoke: `.\.venv\Scripts\python.exe scripts\qa\gui_launcher_wrapper_smoke.py`.
...
- Для короткого отдельного boxplot ОФЗ-ПД используется fallback `jittered strip + summary ticks`: точки разведены по горизонтали, min/median/max показаны отдельными отметками, Q1/Q3 остаются в hover и stats export.
...
HTML-графики организованы по тематическим подпапкам. Source-карта структуры generated outputs находится в [`docs/00_project/outputs_structure.md`](docs/00_project/outputs_structure.md). Generated `outputs/charts/index.md` не является обязательным source artifact и не требуется quality gate.
...
- the empty outputs folder skeleton is kept in Git via `.gitkeep`; generated chart indexes and generated outputs are not required source artifacts.
