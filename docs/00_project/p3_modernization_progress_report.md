...
## 2026-06-23 - Общая UX-доработка GUI launcher

| Дата | Изменение | Проверка | Результат | Ограничения |
|---|---|---|---|---|
| 2026-06-23 | Выполнены UX.1-UX.6 для GUI launcher: на все вкладки добавлен блок `Назначение / Когда использовать / Как запускать / Что изменяет`; основные labels русифицированы; Pipeline получил radio buttons для этапа 0 Минфина и одну основную кнопку запуска; проверки качества, отчеты, release и обслуживание сгруппированы по workflow; нижний preview переименован в `Технические детали выбранного действия` и получил placeholder/mutation/confirm/log details; журнал показывает время старта/завершения и статус; справка дополнена рекомендуемым порядком работы. | `py_compile`; `compileall -q scripts`; `scripts\qa\gui_launcher_smoke.py`; `ofz-gui --smoke-ui`; итоговые CLI help/install checks. | OK: smoke проверяет description blocks, русские labels, Minfin advanced controls, Pipeline stage 0 modes, dangerous confirm gates и запрет arbitrary shell action. | Manual interactive GUI smoke остается операторской проверкой. Dangerous actions, live download, release build и cleanup delete не запускались без отдельного подтверждения. |

## 2026-06-23 - GUI user summaries and result actions

| Дата | Изменение | Проверка | Результат | Ограничения |
|---|---|---|---|---|
| 2026-06-23 | Исправлена UX-модель статусов GUI: добавлен action metadata для `has_results`, result label, user success/failure hints и mutation summary; диагностические actions больше не имеют активной кнопки `Открыть результаты`; user summary отделен от technical log; exit code переименован в технический код завершения; Minfin dry-run/download показывают человекочитаемый итог; command runner передает subprocess UTF-8 env; Pipeline stage 0 получил пояснения режимов. | `py_compile`; `compileall -q scripts`; `scripts\qa\gui_launcher_smoke.py`; `scripts\qa\gui_command_runner_smoke.py`; `ofz-gui --help`; `ofz-gui --smoke-ui`. | OK: smoke проверяет `has_results=false` для diagnostics, `has_results=true` для pipeline/release/BI, UTF-8 env runner, отсутствие старого no-result popup в normal flow и запрет arbitrary shell action. | Manual GUI smoke остается операторской проверкой. Измененные raw/registry/versions в рабочем дереве не относятся к этому этапу и не коммитились. |

## 2026-06-23 - GUI registry labels, layout and cleanup logs fix

| Дата | Изменение | Проверка | Результат | Ограничения |
|---|---|---|---|---|
| 2026-06-23 | Исправлены три runtime/UX замечания GUI: registry mode теперь отображается русскими пользовательскими labels с сохранением internal `off/warn/strict`; окно и нижний блок технических деталей получили более устойчивый layout; runtime logs перенесены из `outputs/reports/launcher/` в `.ofz_launcher/logs/`, чтобы cleanup outputs не удалял активный GUI log. `.ofz_launcher/` добавлен в `.gitignore` и artifact guard. Cleanup summaries уточнены для dry-run/delete и PermissionError. | `py_compile`; `compileall -q scripts`; `scripts\qa\gui_launcher_smoke.py`; `ofz-gui --help`; `ofz-clean-outputs --help`; smoke проверки log root. | OK: GUI smoke проверяет русские registry labels и internal `warn`; command runner smoke пишет log в `.ofz_launcher/logs/`; cleanup helper компилируется. | Manual resize/interactive cleanup delete требует операторской проверки. Existing raw/output changes в рабочем дереве не относятся к этапу и не коммитились. |
