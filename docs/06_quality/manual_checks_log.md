...
## 2026-06-23 - GUI registry labels/layout/cleanup logs runtime fix
...
## 2026-07-02 - Этап G OFZ-PD + key rate chart contract

| Дата | Что проверено | Результат | Ограничения |
|---|---|---|---|
| 2026-07-02 | График `ofz_pd_yield_key_rate` переключен на monthly key rate contract: `key_rate_month_end_pct`, `key_rate_date`, `key_rate_source_rule`, `key_rate_month_is_partial`; chart export больше не использует `key_rate_pct`, `inflation_yoy_pct`, `inflation_target_pct`; hover key rate форматируется с двумя знаками; добавлено примечание про последний доступный день месяца. | OK: прошли `py_compile`, `cbr_key_rate_parser_smoke.py`, `compileall`, `check_text_encoding.py`, `ofz-run`, `cbr_key_rate.py --source html-file --dry-run`, `html_chart_qa.py`, `visual_regression.py --mode auto` для текущего report scope, `ofz_pd_yield_metrics_regression.py`, `ofz-quality --fast`. | Live web CBR не используется; если reference monthly CSV отсутствует, chart flow лениво создает generated reference CSV/JSON из emergency XLSX fallback. Source registry default policy не менялась. Generated outputs, logs, releases, `.ofz_launcher`, `data/processed`, raw CBR XLSX и raw versions не staging. |

## 2026-07-02 - Этап H collision-aware labels для OFZ-PD + key rate
...
| 2026-07-06 | CBR key rate storage workflow переведен на raw latest/version/registry: web update пишет `data/raw/cbr/key_rate_inflation/latest/cbr_key_rate_daily.csv`, meta JSON и registry; GUI status проверяет raw, а chart flow строит monthly key-rate series в памяти из raw daily. | OK: прошли `py_compile`, `compileall`, `check_text_encoding.py`, `cbr_key_rate_parser_smoke.py`, `cbr_raw_status_smoke.py`, `gui_launcher_smoke.py`, `gui_command_runner_smoke.py`, pytest для CBR GUI/parser, live CBR web dry-run вне sandbox (`rows=1894`, `parser=html_table`), controlled update (`write_status=changed`), repeated update (`write_status=unchanged`, version CSV count=1), `cbr_raw_status_smoke.py --check-current`, `ofz-run --report-date 2026-07-01 --retrospective-years 4 --period-type month --aggregation-mode cumulative`, `html_chart_qa.py`, `ofz-quality --fast`. | Raw latest CSV/meta/registry staged как controlled source update; raw versions, generated outputs, logs, releases, `.ofz_launcher`, `data/processed` и local output leftovers не staging. GitHub Actions не проверяются в рамках этой инструкции. |
| 2026-07-06 | CBR freshness detection исправлен: default `To date` стал сегодняшней датой, parser выводит `latest_available_date/latest_available_value` из `table.data`, GUI сравнивает local raw latest date с site latest date и отделяет historical range check. | OK: прошли `py_compile`, `cbr_key_rate_parser_smoke.py`, `cbr_raw_status_smoke.py`, `gui_launcher_smoke.py`, pytest для CBR GUI/parser; live dry-run и controlled update выполняются отдельным операторским шагом с `<TODAY_DD.MM.YYYY>`. | Historical `To date` больше не доказывает текущую актуальность. Raw versions, processed/reference, outputs, logs, releases и `.ofz_launcher` не staging. GitHub Actions не проверяются в рамках этой инструкции. |

## 2026-07-02 - Этап I OFZ-PD single-period boxplot fallback

| Дата | Что проверено | Результат | Ограничения |
|---|---|---|---|
| 2026-07-02 | Отдельный `yield_boxplot_ofz_pd` для короткого годового horizon переведен на fallback `jittered strip + summary ticks`: точки разведены по горизонтали, Q1/Q3 показаны box-shape, min/median/max отдельными ticks и подписями рядом с периодом. | OK: прошли `py_compile`, `ofz_pd_boxplot_single_period_regression.py`, `html_chart_qa.py` и `visual_regression.py --mode auto` для `2026-01-01/year/retrospective_1`; затем восстановлен текущий monthly scope и прошли `ofz-run`, `html_chart_qa.py`, `visual_regression.py --mode auto`, `compileall`, `check_text_encoding.py`, `ofz-quality --fast`. | Изменяется только короткий OFZ-PD boxplot; stats export и основной multi-period boxplot contract сохраняются. Visual regression использовал fallback из-за недоступного screenshot backend в managed-среде. Source registry default policy не менялась: `warn + allow-legacy-raw`. Generated outputs, logs, releases, `.ofz_launcher`, `data/processed`, raw CBR XLSX и raw versions не staging. |
